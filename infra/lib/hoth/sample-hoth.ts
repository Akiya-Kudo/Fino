...
export class Hoth extends BaseStack {
	...
	constructor(scope: Construct, baseName: string, props?: HothProps) {
		...
		const config = {
			...
			lifecycleConfig: {
				transitionToInfrequentAccessDays:
					props?.lifecycleConfig?.transitionToInfrequentAccessDays ?? 30,
				transitionToGlacierDays:
					props?.lifecycleConfig?.transitionToGlacierDays ?? 90,
				transitionToDeepArchiveDays:
					props?.lifecycleConfig?.transitionToDeepArchiveDays ?? 180,
				noncurrentVersionTransitionToGlacierDays:
					props?.lifecycleConfig?.noncurrentVersionTransitionToGlacierDays ??
					30,
				noncurrentVersionExpirationDays:
					props?.lifecycleConfig?.noncurrentVersionExpirationDays ?? 90,
				expiredObjectDeleteMarkerDays:
					props?.lifecycleConfig?.expiredObjectDeleteMarkerDays ?? 1,
			},
		};
		...
		if (config.enableRawLayer) {
			this.rawBucket = this.createDataBucket(
				...
				{
					transitionToInfrequentAccessDays:
						config.lifecycleConfig.transitionToInfrequentAccessDays,
					transitionToGlacierDays:
						config.lifecycleConfig.transitionToGlacierDays,
					transitionToDeepArchiveDays:
						config.lifecycleConfig.transitionToDeepArchiveDays,
					noncurrentVersionTransitionToGlacierDays:
						config.lifecycleConfig.noncurrentVersionTransitionToGlacierDays,
					noncurrentVersionExpirationDays:
						config.lifecycleConfig.noncurrentVersionExpirationDays,
				},
			);
		...
		if (config.enableProcessedLayer) {
			this.processedBucket = this.createDataBucket(
				...
				{
					transitionToInfrequentAccessDays: Math.floor(
						config.lifecycleConfig.transitionToInfrequentAccessDays / 2,
					),
					transitionToGlacierDays: Math.floor(
						config.lifecycleConfig.transitionToGlacierDays * 0.7,
					),
					transitionToDeepArchiveDays:
						config.lifecycleConfig.transitionToDeepArchiveDays,
					noncurrentVersionTransitionToGlacierDays: Math.floor(
						config.lifecycleConfig.noncurrentVersionTransitionToGlacierDays / 2,
					),
					noncurrentVersionExpirationDays: Math.floor(
						config.lifecycleConfig.noncurrentVersionExpirationDays * 0.7,
					),
				},
			);
		...
		if (config.enableCuratedLayer) {
			this.curatedBucket = this.createDataBucket(
				...
				{
					transitionToInfrequentAccessDays: Math.floor(
						config.lifecycleConfig.transitionToInfrequentAccessDays / 3,
					),
					transitionToGlacierDays: Math.floor(
						config.lifecycleConfig.transitionToGlacierDays / 2,
					),
					transitionToDeepArchiveDays:
						config.lifecycleConfig.transitionToGlacierDays, // より早くアーカイブ
					noncurrentVersionTransitionToGlacierDays: 7,
					noncurrentVersionExpirationDays: Math.floor(
						config.lifecycleConfig.noncurrentVersionExpirationDays / 2,
					),
				},
			);
	...
	private createDataBucket(
		...
		const bucket = new s3.Bucket(
			this,
			`${this.capitalizeFirst(layerName)}Bucket`,
			{
						...
						transitions: [
							{
								...
								transitionAfter: cdk.Duration.days(
									lifecycleConfig.transitionToInfrequentAccessDays,
								),
							},
							{
								storageClass: s3.StorageClass.GLACIER,
								transitionAfter: cdk.Duration.days(
									lifecycleConfig.transitionToGlacierDays,
								),
							},
							{
								storageClass: s3.StorageClass.DEEP_ARCHIVE,
								transitionAfter: cdk.Duration.days(
									lifecycleConfig.transitionToDeepArchiveDays,
								),
							},
						],
					},
						...
						noncurrentVersionTransitions: [
							{
								storageClass: s3.StorageClass.GLACIER,
								transitionAfter: cdk.Duration.days(
									lifecycleConfig.noncurrentVersionTransitionToGlacierDays,
								),
							},
						],
						noncurrentVersionExpiration: cdk.Duration.days(
							lifecycleConfig.noncurrentVersionExpirationDays,
						),
					},
					{
