...
s3_warehouse_path = os.environ.get(
    "S3_WAREHOUSE_PATH", "s3://fino-lakehouse/warehouse"
)
    ...
    for document_id in document_ids:
        try:
            ...
            if item and item.get("ingestion_state") == "READY":
                ...
                logger.info(
                    f"Document {document_id} is not READY (state: {item.get('ingestion_state') if item else 'NOT_FOUND'})"
                )
        except Exception as e:
            logger.error(
                f"Error getting document {document_id} from DynamoDB: {e}"
            )
            continue
    ...
    for document_id in ready_document_ids:
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to save to Iceberg for document {document_id}: {iceberg_error}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to update state to COMPLETED for {document_id}: {db_error}"
                )
                # データは保存済みなので、成功としてカウント（冪等性のため）
                logger.warning(
                    f"Document {document_id} saved to Iceberg but state update failed"
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to update error state for {document_id}: {update_error}"
                )
            continue
...
def _update_ingestion_state(
    partition_key: str, document_id: str, state: str
) -> None:
...
def _fetch_edinet_document(
    client: EdinetClient, document_id: str
) -> Optional[bytes]:
    """Fetch document from EDINET API"""
    try:
...
def _save_to_iceberg(
    sec_code: str, document_id: str, document_data: bytes
) -> None:
    ...
    try:
        ...
        try:
            ...
            if df.empty:
                logger.warning(
                    f"Document {document_id} contains no data, skipping"
                )
                return
            ...
            try:
                ...
                logger.info(
                    f"Table {full_table_name} does not exist, creating new table"
                )
                iceberg_table = _create_iceberg_table(
                    catalog, namespace, table_name, df
                )
            ...
            logger.info(
                f"Successfully saved document {document_id} to Iceberg table {full_table_name}"
            )

        except pd.errors.EmptyDataError:
            logger.warning(
                f"Document {document_id} is empty or invalid CSV format"
            )
            raise
        except Exception as e:
            logger.error(
                f"Error processing CSV data for document {document_id}: {e}"
            )
            raise
        ...
        if tmp_file_path and Path(tmp_file_path).exists():
            try:
                ...
                logger.warning(
                    f"Failed to delete temporary file {tmp_file_path}: {cleanup_error}"
                )


def _create_iceberg_table(
    catalog, namespace: str, table_name: str, sample_df: pd.DataFrame
):
    ...
    try:
        ...
        logger.error(
            f"Error creating Iceberg table {namespace}.{table_name}: {e}"
        )
        raise
